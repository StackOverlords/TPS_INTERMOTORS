...
import { Clock, HelpCircle, Loader2, Plus, Settings, User } from "lucide-react";
...
import type {
  OrderGetAll,
  OrderGetAllResponse,
} from "../../types/orderGet.types";

interface OrderSelectorTableProps {
...
const OrderSelectorTable: React.FC<OrderSelectorTableProps> = ({
  data,
  orders,
  filters,
  setPage,
            ...
            <TooltipWrapper
              ...
              tooltip={
                <p className="flex gap-1">
                  Presiona <Kbd>enter</Kbd> para seleccionar el pedido
                </p>
              }
            >
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="space-y-1 flex flex-col">
              <span
                className={`${!proveedor ? "italic text-muted-foreground" : "font-medium text-foreground"}`}
              >
                ...
                <div className="text-xs text-muted-foreground flex items-center gap-3">
                  ...
                  {proveedor.contacto && (
                    <div className="flex gap-1 items-center">
                      <User className="size-3" /> {proveedor.contacto}
                    </div>
                  )}
                </div>
              )}
  ...
  const { table } = useCustomTable({
    ...
    persistenceKey: `orders-selector-table-${user?.name}`,
  ...
  const handleRowDoubleClick = (order: OrderGetAll) => {
    onSelectOrder(order);
  };
  ...
  return (
    <section className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 min-h-0">
        {isInfiniteScroll ? (
          <div
            id="orders-selector-scroll-container"
          ...
          >
            <InfiniteScroll
              ...
              scrollableTarget="orders-selector-scroll-container"
            >
              <CustomizableTable
                table={table}
                isError={isError}
...
export default OrderSelectorTable;
