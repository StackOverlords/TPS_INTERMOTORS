...
import { AccountPayableDetailModal } from "../components/accountPayableDetail/AccountPayableDetailModal";
import AccountPayableFilters from "../components/accountsPayableList/AccountPayableFilters";
import { useAccountsPayablePaginated } from "../hooks/queries/useAccountsPayablePaginated";
import { useAccountPayableFilters } from "../hooks/useAccountPayableFilters";
import type { AccountPayable } from "../types/accountPayable";

const AccountsPayableListScreen = () => {
  ...
  const [selectedAccountPayableId, setSelectedAccountPayableId] = useState<number | null>(null);
  ...
  const {
    ...
    setPageSize,
    applyFilters,
  } = useAccountPayableFilters(Number(selectedBranchId) || 1);
  ...
  const {
    ...
    isFetching,
    isError,
    refetch: refetchAccountsPayable,
    isRefetching: isRefetchingAccountsPayable,
  } = useAccountsPayablePaginated(activeFilters);
  ...
  useEffect(() => {
    ...
    if (isInfiniteScroll && filters.pagina && filters.pagina > 1) {
      setAccountsPayable((prev) => {
        const newAccounts = accountsPayableData.data.filter(
          (newAccount: any) =>
            !prev.some((existingAccount) => existingAccount.id === newAccount.id)
        );
        return [...prev, ...newAccounts];
      });
    } else {
      setAccountsPayable(accountsPayableData.data);
    }
  }, [accountsPayableData?.data, isInfiniteScroll, filters.pagina, error, isFetching]);
  ...
  const columns = useMemo<ColumnDef<AccountPayable>[]>(
    () => [
      ...
      },
      {
        ...
        enableHiding: true,
        cell: ({ getValue }) => (
          <div className="text-sm">{getValue<string | null>() || formatCell(null)}</div>
        ),
      },
      {
        accessorKey: "fecha",
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <div className="text-center font-medium">
              {plazoPago ? new Date(plazoPago).toLocaleDateString("es-ES") : formatCell(null)}
            </div>
          );
        },
      },
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <div className="text-center">
              <Badge variant={getSaldoColor(saldo)} className="rounded font-bold">
  ...
  const { table } = useCustomTable({
    ...
    persistenceKey: `accounts-payable-table-${user?.name}`,
    persistColumnVisibility: true,
    persistColumnOrder: true,
  });

  const { selectedIndex, setSelectedIndex, isFocused, hotkeys } =
    useKeyboardNavigation<AccountPayable, HTMLTableElement>({
      items: accountsPayable,
      containerRef: tableRef,
      isDragging: isDraggingColumn,
      onPrimaryAction: (account) => {
        if (!modalOpen) {
          setSelectedAccountPayableId(account.id);
          setModalOpen(true);
        }
      },
  ...
  const handleRefetchAccountsPayable = () => {
    refetchAccountsPayable();
  };
...
export default AccountsPayableListScreen;
