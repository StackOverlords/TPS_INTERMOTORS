...
interface KardexReportFiltersPanelProps {
  filters: KardexReportFilters;
  onFiltersChange: <K extends keyof KardexReportFilters>(
    key: K,
  ...
  setSelectedProductInfo: React.Dispatch<
    React.SetStateAction<{ id: number; nombre: string } | null>
  >;
}

export function KardexReportFiltersPanel({
  filters,
  onFiltersChange,
  onRefresh,
  onExport,
  ...
  // Hook para ventana secundaria de productos
  const productWindow = useProductSelectorWindow({
    context: "kardex",
    instanceId: "kardex-report",
    simpleMode: true,
    multiSelect: false,
    validateStock: false,
    onProductSelect: (product: ProductGet) => {
      onFiltersChange("producto", product.id);
      // Guardamos la info del producto seleccionado desde el buscador
      setSelectedProductInfo({
        id: product.codigo_interno,
        nombre: product.descripcion || `Producto #${product.codigo_interno}`,
      });
    },
  });
  ...
  return (
    <div className="space-y-2">
      <div ref={containerRef} className="grid grid-cols-1 md:grid-cols-4 gap-2">
        {/* Fecha Inicio */}
      ...
      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            disabled={loading || isDownloading || !filters.producto}
          >
            <Download
          ...
          {searchMode === "manual" && onSearch && (
            <Button
              onClick={onSearch}
              disabled={isFetching || isDownloading || !filters.producto}
            >
              {isFetching ? (
