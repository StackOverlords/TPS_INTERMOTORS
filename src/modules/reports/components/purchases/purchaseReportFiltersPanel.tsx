...
import type { PurchaseReportFiltersState } from "../../hooks/purchases/usePurchaseReportFilters";
import { EditableQuantity } from "@/modules/shoppingCart/components/editableQuantity";

interface PurchaseReportFiltersPanelProps {
  filters: PurchaseReportFiltersState;
  onFiltersChange: <K extends keyof PurchaseReportFiltersState>(
    key: K,
  ...
  /** Mostrar campo ranking (para reportes de más comprado y mayor costo) */
  showRanking?: boolean;
}

export function PurchaseReportFiltersPanel({
  filters,
  onFiltersChange,
  onRefresh,
  onExport,
  ...
  isDownloading = false,
  showRanking = false,
}: PurchaseReportFiltersPanelProps) {
  ...
  return (
    <div className="space-y-2">
      <div
        ref={containerRef}
        className={`grid grid-cols-1 gap-2 ${showRanking ? "md:grid-cols-4" : "md:grid-cols-3"}`}
      >
        {/* Fecha Inicio */}
