...
const BrandsScreen = () => {
  ...
  const { mutate: handleCreateBrand, isPending: isCreating } = useCreateBrand();

  const { mutate: handleUpdateBrand, isPending: isUpdating } = useUpdateBrand();
  ...
  const handleCreateSubmit = useCallback(
    createForm.handleSubmit(async (data: CreateBrand) => {
  ...
  const handleUpdateSubmit = useCallback(
    updateForm.handleSubmit(async (data: UpdateBrand) => {
      if (!editingId) {
        showErrorToast({
          title: "Error al modificar marca",
          description:
            "No se pudo modificar la marca. Por favor, intenta nuevamente",
          duration: 5000,
        });
        return;
      }
      ...
      handleUpdateBrand(
        { data, id: editingId },
        {
          ...
          onError: (error: unknown) => {
            handleError({
              error,
              customTitle: "No se pudo modificar la marca",
            });
          },
        }
      );
  ...
  const { table } = useCustomTable({
    ...
    persistenceKey: `brands-table-${user?.name}`,
  ...
  return (
    <main className="w-full max-w-5xl mx-auto h-full p-2 gap-2 flex flex-col">
      <ProtectedAction
        permission={PERMISSIONS.SIS.ADM_MARCAS}
            ...
            <div className="flex-1 min-h-0">
              <div className="h-full overflow-auto">
                <CustomizableTable
                  ...
                  errorMessage="Ocurrió un error al cargar las marcas."
