...
const MEASUREMENTS_DIALOG_CONFIG: DialogConfig = {
  ...
  screen_path: "/dashboard/settings/measurements",
};
...
const MeasurementsScreen = () => {
  ...
  const { mutate: handleCreateMeasurement, isPending: isCreating } =
    useCreateMeasurement();

  const { mutate: handleUpdateMeasurement, isPending: isUpdating } =
  ...
  const totalRecords = useMemo(
    () => measurementsData?.meta?.total || 0,
    [measurementsData?.meta?.total]
  );
  const isRefreshing = useMemo(
    () => isRefetchingMeasurementsData || isFetchingMeasurementsData,
    [isRefetchingMeasurementsData, isFetchingMeasurementsData]
  );

  useTabEffect(
    MEASUREMENTS_DIALOG_CONFIG.screen_path,
    () => {
  ...
  const handleCreateSubmit = useCallback(
    createForm.handleSubmit(async (data: CreateMeasurement) => {
  ...
  const handleUpdateSubmit = useCallback(
    updateForm.handleSubmit(async (data: UpdateMeasurement) => {
      if (!editingId) {
        showErrorToast({
          title: "Error al modificar medida",
          description:
            "No se pudo modificar la medida. Por favor, intenta nuevamente",
          duration: 5000,
        });
        return;
      }
      ...
      handleUpdateMeasurement(
        { data, id: editingId },
        {
          ...
          onError: (error: unknown) => {
            handleError({
              error,
              customTitle: "No se pudo modificar la medida",
            });
          },
        }
      );
  ...
  useEffect(() => {
    ...
    isErrorMeasurementById,
    errorMeasurementById,
    handleError,
    handleDialogToggle,
  ]);
  ...
  return (
    <main className="w-full max-w-5xl mx-auto h-full p-2 gap-2 flex flex-col">
      <ProtectedAction
        permission={PERMISSIONS.SIS.ADM_UNIDADES}
