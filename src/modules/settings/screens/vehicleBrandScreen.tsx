...
import type {
  ...
  UpdateVehicleBrand,
  VehicleBrand,
} from "../types/vehicleBrand.types";
import {
  CreateVehicleBrandSchema,
  UpdateVehicleBrandSchema,
} from "../schemas/vehicleBrand.schema";
...
const VehicleBrandsScreen = () => {
  ...
  const { mutate: handleCreateVehicleBrand, isPending: isCreating } =
    useCreateVehicleBrand();

  const { mutate: handleUpdateVehicleBrand, isPending: isUpdating } =
  ...
  const handleDeleteError = useCallback(
    (error: unknown, id: number) => {
      handleError({
        error,
        customTitle: `Error al eliminar marca de vehículo #${id}`,
      });
    },
    [handleError]
  );

  const { mutate: deleteVehicleBrand, isPending: isDeleting } =
  ...
  const totalRecords = useMemo(
    () => vehicleBrandsData?.meta?.total || 0,
    [vehicleBrandsData?.meta?.total]
  );
  ...
  const handleCreateSubmit = useCallback(
    createForm.handleSubmit(async (data: CreateVehicleBrand) => {
      handleCreateVehicleBrand(data, {
        ...
        onError: (error: unknown) => {
          handleError({
            error,
            customTitle: "No se pudo agregar la marca de vehículo",
          });
        },
      });
    }),
  ...
  const handleUpdateSubmit = useCallback(
    updateForm.handleSubmit(async (data: UpdateVehicleBrand) => {
      if (!editingId) {
        showErrorToast({
          title: "Error al modificar marca de vehículo",
          description:
            "No se pudo modificar la marca de vehículo. Por favor, intenta nuevamente",
          duration: 5000,
        });
        return;
      }

      handleUpdateVehicleBrand(
        { data, id: editingId },
        {
          onSuccess: () => {
            showSuccessToast({
          ...
          onError: (error: unknown) => {
            handleError({
              error,
              customTitle: "No se pudo modificar la marca de vehículo",
            });
          },
        }
      );
  ...
  useEffect(() => {
    ...
    isErrorVehicleBrandById,
    errorVehicleBrandById,
    handleError,
    handleDialogToggle,
  ]);
  ...
  const { table } = useCustomTable({
    ...
    persistenceKey: `vehicleBrand-table-${user?.name}`,
  ...
  const {
    ...
    isFocused,
    // hotkeys
  } = useKeyboardNavigation<VehicleBrand, HTMLTableElement>({
    items: vehicleBrandsData?.data || [],
    containerRef: tableRef,
    isDragging: isDraggingColumn,
    onPrimaryAction: (vehicleBrand) => {
      handleEditVehicleBrand(vehicleBrand.id);
    },
    getItemId: (vehicleBrand) => vehicleBrand.id,
  });
            ...
            <div className="flex-1 min-h-0">
              <div className="h-full overflow-auto">
                <CustomizableTable
                  ...
                  errorMessage="Ocurrió un error al cargar las marcas de vehículo."
                  noDataMessage="No se encontraron marcas de vehículo."
