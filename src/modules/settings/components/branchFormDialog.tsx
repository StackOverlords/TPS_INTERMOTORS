...
const BranchFormDialog: React.FC<BranchFormDialogProps> = ({
  ...
  const { mutate: handleCreateBranch, isPending: isCreating } =
    useCreateBranch();

  const { mutate: handleUpdateBranch, isPending: isUpdating } =
    useUpdateBranch();
  ...
  useEffect(() => {
    ...
    if (branchById && isEditing) {
      ...
      if (
        branchById.imagen &&
        typeof branchById.imagen === "string" &&
        branchById.imagen.trim() !== ""
      ) {
  ...
  const handleCreateSubmit = useCallback(
    createForm.handleSubmit(async (data: CreateBranch) => {
  ...
  const handleUpdateSubmit = useCallback(
    updateForm.handleSubmit(async (data: UpdateBranch) => {
      if (!editingId) {
        showErrorToast({
          title: "Error al modificar sucursal",
          description:
            "No se pudo modificar la sucursal. Por favor, intenta nuevamente",
          duration: 5000,
        });
        return;
      }
      ...
      handleUpdateBranch(
        { data, id: editingId },
        {
          ...
          onError: (error: unknown) => {
            handleError({
              error,
              customTitle: "No se pudo modificar la sucursal",
            });
          },
        }
      );
  ...
  const onSubmit = useMemo(
    () => (isEditing ? handleUpdateSubmit : handleCreateSubmit),
    [isEditing, handleUpdateSubmit, handleCreateSubmit]
  );
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            ...
            {isEditing && editingId && (
              <span className="text-muted-foreground ml-2">#{editingId}</span>
            )}
          </DialogTitle>
          <DialogDescription>
            {isEditing ? "Modifica los datos de la" : "Agrega una nueva"}{" "}
            sucursal
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
              ...
              {isEditing && isLoadingBranchById ? (
                ...
                <>
                  <Input
                    id="sigla"
                    {...(isEditing
                      ? updateForm.register("sigla")
                      : createForm.register("sigla"))}
            ...
            {isEditing && isLoadingBranchById ? (
              ...
              <>
                <Input
                  id="nombre_comercial"
                  {...(isEditing
                    ? updateForm.register("nombre_comercial")
                    : createForm.register("nombre_comercial"))}
                ...
                <div>
                  ...
                  {isEditing && isLoadingBranchById ? (
                    ...
                    <Input
                      id="fax"
                      {...(isEditing
                        ? updateForm.register("fax")
                        : createForm.register("fax"))}
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button type="submit" disabled={isLoadingBranchById || isSaving}>
              {isSaving ? (
                <>
