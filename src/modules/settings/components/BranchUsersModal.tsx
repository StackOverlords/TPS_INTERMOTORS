...
export const BranchUsersModal = ({
  branchId,
  branchName,
  open,
  onOpenChange,
}: BranchUsersModalProps) => {
  ...
  const { data: rolesData } = useGetBranchRoles();

  const { data: allUsersData } = useUsersPaginated({
    pagina: 1,
    pagina_registros: 1000,
  });

  const { mutate: addUser, isPending: isAddingUser } = useAddUserToBranch();
  ...
  const availableUsers = useMemo(() => {
    ...
    const branchUserIds = new Set(
      branchUsersData.data.map((u) => u.id_usuario)
    );
  ...
  const handleAddUser = useCallback(() => {
    ...
    addUser(
      {
        branchId,
        data: { usuario_id: Number(selectedUserId), rol: selectedRole },
      },
      {
        onSuccess: () => {
          showSuccessToast({
