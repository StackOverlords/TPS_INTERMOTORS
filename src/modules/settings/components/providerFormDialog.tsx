...
const ProviderFormDialog: React.FC<ProviderFormDialogProps> = ({
  ...
  const { data: providerById, isLoading: isLoadingProviderById } =
    useGetProviderById(editingId || 0);

  const { mutate: handleCreateProvider, isPending: isCreating } =
    useCreateProvider();

  const { mutate: handleUpdateProvider, isPending: isUpdating } =
    useUpdateProvider();
  ...
  const { data: countriesData, isLoading: isLoadingCountries } =
    useGetCountriesProviders();
  const { data: statesData, isLoading: isLoadingStates } =
    useGetStatesProviders(selectedCountryId);
  const { data: citiesData, isLoading: isLoadingCities } =
    useGetCitiesProviders(selectedStateId);
            ...
            handleUpdateProvider(
              ...
              },
              {
                ...
                onError: (error: unknown) => {
                  handleError({
                    error,
                    customTitle: "No se pudo actualizar el proveedor",
                  });
                },
              }
            );
        ...
        : createForm.handleSubmit((data: any) => {
            handleCreateProvider(data, {
              ...
              onError: (error: unknown) => {
                handleError({
                  error,
                  customTitle: "No se pudo crear el proveedor",
                });
              },
            });
          });
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div>
                ...
                {isEditing && isLoadingProviderById ? (
                  ...
                  <Input
                    id="contacto"
                    {...(isEditing
                      ? updateForm.register("contacto")
                      : createForm.register("contacto"))}
          ...
          <div className="pt-2 ">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                {/* Estado/Provincia */}
                <div>
                  <Label htmlFor="provincia_departamento_id">
                    Estado/Provincia *
                  </Label>
                  {isEditing && isLoadingProviderById ? (
                    <div className="flex items-center justify-start px-2 h-10 gap-3 rounded-md animate-pulse bg-accent">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button type="submit" disabled={isLoadingProviderById || isSaving}>
              {isSaving ? (
                <>
