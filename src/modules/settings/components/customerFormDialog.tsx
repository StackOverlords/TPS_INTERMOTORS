...
const CustomerFormDialog: React.FC<CustomerFormDialogProps> = ({
  ...
  const { data: customerById, isLoading: isLoadingCustomerById } =
    useGetCustomerById(editingId || 0);

  const { mutate: handleCreateCustomer, isPending: isCreating } =
    useCreateCustomer();

  const { mutate: handleUpdateCustomer, isPending: isUpdating } =
    useUpdateCustomer();
  ...
  const { data: countriesData, isLoading: isLoadingCountries } =
    useGetCountriesCustomers();
  const { data: statesData, isLoading: isLoadingStates } =
    useGetStatesCustomers(selectedCountryId);
  const { data: citiesData, isLoading: isLoadingCities } =
    useGetCitiesCustomers(selectedStateId);
            ...
            handleUpdateCustomer(
              ...
              },
              {
                ...
                onError: (error: unknown) => {
                  handleError({
                    error,
                    customTitle: "No se pudo actualizar el cliente",
                  });
                },
              }
            );
        ...
        : createForm.handleSubmit((data) => {
            handleCreateCustomer(data, {
              ...
              onError: (error: unknown) => {
                handleError({
                  error,
                  customTitle: "No se pudo crear el cliente",
                });
              },
            });
          });
                ...
                <div>
                  ...
                  {isEditing && isLoadingCustomerById ? (
                    ...
                    <Input
                      ...
                      type="number"
                      {...(isEditing
                        ? updateForm.register("dias_plazo", {
                            valueAsNumber: true,
                          })
                        : createForm.register("dias_plazo", {
                            valueAsNumber: true,
                          }))}
                      placeholder="0"
                      disabled={isLoadingCustomerById || isSaving}
