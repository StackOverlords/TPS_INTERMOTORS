...
const CategoryListTable: React.FC<CategoryListTableProps> = ({
  ...
  const { mutate: handleCreateCategory, isPending: isCreating } =
    useCreateCategory();

  const { mutate: handleUpdateCategory, isPending: isUpdating } =
    useUpdateCategory();
  ...
  const handleCreateSubmit = useCallback(
    createForm.handleSubmit(async (data: CreateCategory) => {
      handleCreateCategory(data, {
        ...
        onError: (error: unknown) => {
          handleError({
            error,
            customTitle: "No se pudo agregar la categoría",
          });
        },
      });
    }),
  ...
  const handleUpdateSubmit = useCallback(
    updateForm.handleSubmit(async (data: UpdateCategory) => {
      if (!editingId) {
        showErrorToast({
          title: "Error al modificar categoría",
          description:
            "No se pudo modificar la categoría. Por favor, intenta nuevamente",
          duration: 5000,
        });
        return;
      }
      ...
      handleUpdateCategory(
        { data, id: editingId },
        {
          ...
          onError: (error: unknown) => {
            handleError({
              error,
              customTitle: "No se pudo modificar la categoría",
            });
          },
        }
      );
                      ...
                      {subcategories
                        ...
                        .map((sub, index) => (
                          ...
                          >
                            {sub.subcategoria}
                            {index <
                              Math.min(previewLimit - 1, totalItems - 1) && ","}
                          </span>
                        ))}
                      {hasMore && (
                ...
                <div className="mt-3 ml-9 border-l-2 border-purple-100 pl-3">
                  <div
                    className={cn(
                      "space-y-1",
                      totalItems > 6 &&
                        "max-h-48 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-purple-200 scrollbar-track-transparent"
                    )}
                  >
