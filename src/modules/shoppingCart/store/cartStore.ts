        ...
        (set, get) => ({
          ...
          addItem: (product) => {
            ...
            if (existing) {
              ...
              if (
                (mode === "sale-permissive" || mode === "quote") &&
              ...
              const quantity = 1;
              const subtotal = multiplyPrecise(basePrice, quantity);
          ...
          setDiscountAmount: (amount) => {
            ...
            const validAmount = minPrecise(maxPrecise(0, amount), subtotal);
            const percent = isGreaterThanZero(subtotal)
              ? calculatePercent(validAmount, subtotal)
              : 0;
          ...
          setDiscountPercent: (percent) => {
            const subtotal = get().getCartSubtotal();
            const validPercent = minPrecise(maxPrecise(0, percent), 100);
            const amount = calculateAmountFromPercent(validPercent, subtotal);
          ...
          recalculateDiscount: () => {
            ...
            if (discountMode === "percent") {
              ...
              const validAmount = minPrecise(discountAmount, newSubtotal);
              const newPercent = isGreaterThanZero(newSubtotal)
                ? calculatePercent(validAmount, newSubtotal)
                : 0;
              set({
                discountAmount: validAmount,
          ...
          getCartTotal: () => {
            ...
            if (discountAmount && isGreaterThanZero(discountAmount)) {
              return maxPrecise(0, subtractPrecise(subtotal, discountAmount));
            }
...
export const calculateSubtotal = (items: CartItem[]) => {
  ...
  const subtotals = items.map((item) => {
    return multiplyPrecise(item.customPrice, item.quantity);
  });
...
export const calculateDiscountAmount = (
  ...
  if (discount.percent) {
    return calculateAmountFromPercent(discount.percent, subtotal);
  }
...
export const getModeLabel = (mode: CartMode): string => {
  switch (mode) {
    case "sale-strict":
      return "Venta Estricta";
    case "sale-permissive":
      return "Venta Permisiva";
    case "quote":
      return "Cotización";
    default:
      return mode;
