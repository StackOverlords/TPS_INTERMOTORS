...
function ReturnDetailTableInner<T extends ReturnDetailUnion>(
  {
    ...
    isSaving = false,
    isEditMode = false,
  }: ReturnDetailTableProps<T>,
  ref: React.Ref<ReturnDetailTableRef>
) {
  ...
  const { mutate: deleteReturnDetail, isPending: isDeleting } =
  ...
  const handleRemoveItem = (item: T) => {
    ...
    const isNew =
      !("almacen_out_dev_det_id" in item) || !item.almacen_out_dev_det_id;
    ...
    if (isNew) {
      ...
      handleOpenDeleteAlert(
        (item as UIReturnDetailUpdate).almacen_out_dev_det_id ?? undefined
      );
    }
  };
    ...
    if (isEditMode) {
      baseColumns.push({
        ...
        cell: ({ row, getValue }) => {
          ...
          const isNew =
            !("almacen_out_dev_det_id" in row.original) ||
            !row.original.almacen_out_dev_det_id;
          return (
        ...
        cell: ({ getValue, row }) => {
          ...
          return (
            <Input
              ...
              onChange={(value) =>
                onUpdateComentario(
                  row.original.almacen_out_det_id,
                  value.target.value
                )
              }
              disabled={isReadOnly || isSaving}
...
const ReturnDetailTable = forwardRef(
  ReturnDetailTableInner
) as React.ForwardRefExoticComponent<
  ReturnDetailTableProps<ReturnDetailUnion> &
    React.RefAttributes<ReturnDetailTableRef>
>;
