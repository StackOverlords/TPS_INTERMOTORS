...
import { Calculator, CalendarDays, Check, Minus, Plus, Search, User } from "lucide-react";
...
interface SelectSalesReturnModalProps {
    ...
    selectedProducts: { almacen_out_det_id: number, cantidad: number, comentario?: string | null }[];
    onConfirm?: (changes: Array<{
...
const SelectSalesReturnModal: React.FC<SelectSalesReturnModalProps> = ({
    ...
    const [tempSelections, setTempSelections] = useState<Map<number, { cantidad: number, comentario: string }>>(new Map());

    // Estado inicial para comparar cambios
    const [initialSelections, setInitialSelections] = useState<Map<number, { cantidad: number, comentario: string }>>(new Map());
    ...
    useEffect(() => {
        if (isDialogOpen && selectedProducts) {
            const initialMap = new Map<number, { cantidad: number, comentario: string }>();

            // Solo inicializar con productos de ESTA venta
            selectedProducts
                .filter(item => {
                    // Verificar si el producto pertenece a esta venta
                    const belongsToSale = saleData?.detalles.some(d => d.id === item.almacen_out_det_id);
    ...
    const getTempQuantity = useCallback((productId: number): number => {
        return tempSelections.get(productId)?.cantidad || 0;
    }, [tempSelections]);

    // Agregar una unidad temporal
    const handleAddTemp = useCallback((product: SaleItemGetById) => {
    ...
    const handleConfirm = useCallback(() => {
        ...
        tempSelections.forEach((tempData, detailId) => {
            ...
            const hasChanged = isNew || (initialData && initialData.cantidad !== tempData.cantidad);
                ...
                return (
                    <div className="space-y-0.5">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                            ...
                            {product.marca && (
                                <Badge variant="outline" title="Marca" className="text-[10px] border-gray-300">
                                    {product.marca.marca}
                                </Badge>
                            )}
                        </div>
    ...
    const { tempTotalItems, totalChanges } = useMemo(() => {
        const totalItems = Array.from(tempSelections.values()).reduce((sum, data) => sum + data.cantidad, 0);
