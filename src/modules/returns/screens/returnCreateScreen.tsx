...
import { CornerUpLeft, Loader2, Plus, Save, ShieldAlert, Undo2 } from "lucide-react";
import {
  Card,
  CardContent,
  CardHeader,
...
import type {
  ReturnCreate,
  UIReturnDetailCreate,
} from "../types/returnCreate.types";
...
const ReturnCreateScreen = () => {
  ...
  const { data: returnTypesData } = useReturnTypes();
  ...
  const handleNewReturn = (canClearDetails = true) => {
    ...
    reset({
      ...
      nro_comprobante: "",
      motivo_devolucion:
        returnTypesData?.[0]?.id || currentValues.motivo_devolucion,
      comentarios: "",
    ...
    createReturn(dataToSend, {
      onSuccess: (createdReturn) => {
        showSuccessToast({
          ...
          description: `Devolución #${createdReturn.nro} realizada con éxito`,
          duration: 2000,
        });

        //   editar tab con la cotizacion creada
  ...
  useEffect(() => {
    if (
      !user?._id &&
      returnResponsiblesData &&
      returnResponsiblesData.data.length > 0
    ) {
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <Select
                                ...
                                value={
                                  field.value || returnTypesData?.[0]?.id || ""
                                }
                              >
                                <SelectTrigger>
                            ...
                            <ProtectedAction
                              ...
                              fallback={
                                <TooltipButton
                                  ...
                                  tooltip={
                                    <span>
                                      No tienes permisos para realizar esta
                                      acción
                                    </span>
                                  }
                                >
                                  <ShieldAlert></ShieldAlert> Sin Permisos
                                </TooltipButton>
                              }
                            >
                            <TooltipButton
