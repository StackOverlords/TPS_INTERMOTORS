...
/**
 * Selector de detalles de venta, AGNÓSTICO AL CONTENEDOR.
 *
 * Recibe su configuración por props y avisa por callbacks. Ver el patrón en
 * `modules/orders/components/orderSelector/OrderSelector.tsx`.
 */
export interface SaleDetailSelectorConfig {
  /**
   * Identificador de esta instancia. Aísla los filtros persistidos entre
   * selectores abiertos a la vez. Reemplaza al viejo `windowId`, que era lo
   * único para lo que se usaba.
   */
  instanceId: string;
  context: string;
  mode: "create" | "edit";
...
export interface SaleDetailSelectorProps {
  config: SaleDetailSelectorConfig;
  /** Cambios confirmados por el usuario. El contenedor decide qué hacer. */
...
export const SaleDetailSelector = ({
  config,
  onChangesApplied,
  onClose,
}: SaleDetailSelectorProps) => {
  const tableRef = useRef<ReturnDetailTableRef>(null);

  const STORAGE_KEY = config.instanceId + "-sale-detail-filters";
  ...
  const confirmAndClose = useCallback(
    ...
    [isConfirming, initialItems, calculateChanges, onChangesApplied]
  );
