      ...
      },
      {
        ...
        cell: ({ getValue, row }) => {
          const porcentajeDescuento =
            typeof row.original.porcentaje_descuento === "string"
              ? parseFloat(row.original.porcentaje_descuento)
              : row.original.porcentaje_descuento;
          const discountPercent = isFinite(porcentajeDescuento)
    ...
    <section className="border border-border rounded-lg bg-background flex-1 flex flex-col overflow-hidden">
      <header className="p-2 border-b border-border flex-shrink-0 space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {filteredSaleItems.length === 1 ? "producto" : "productos"} en total
        </p>
