...
} from "@/modules/shoppingCart/components/tableShoppingCart";
import { useCartWithUtils } from "@/modules/shoppingCart/hooks/useCartWithUtils";
import authSDK from "@/services/sdk-simple-auth";
import { useBranchStore } from "@/states/branchStore";
import { zodResolver } from "@hookform/resolvers/zod";
import { parse } from "date-fns";
import { CornerUpLeft, Dot, FileText, Plus, ShoppingCart, Trash2 } from "lucide-react";
...
import type { Sale, SaleDetail, SalePaymentMethod } from "../types/sale";
...
const CreateSaleScreen = () => {
  ...
  const { mutate: createSale, isPending: isSaving } = useCreateSale();
  ...
  const handleManualModeChange = useCallback(
    (newMode: "sale-strict" | "sale-permissive") => {
  ...
  const handleNewSale = useCallback(
    (canClearCart = true) => {
      ...
      reset({
        ...
        tipo_venta: saleTypesData?.[0]?.code || "",
        forma_venta: saleModalitiesData?.[0]?.code || "",
        forma_pago: salePaymentTypesData?.[0]?.code || "",
        comentario: "",
        plazo_pago: "",
        vehiculo: "",
        nro_motor: "",
      ...
      onSuccess: (createdSale) => {
        ...
        if (currentTab) {
          updateTab(currentTab.id, {
            path: `/dashboard/sales/${createdSale.id}/update`,
            title: `Editar venta: ${createdSale.id}`,
            createdTempData: {
