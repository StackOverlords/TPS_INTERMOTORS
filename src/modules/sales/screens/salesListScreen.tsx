...
const SalesListScreen = () => {
  ...
  const { config, isFeatureEnabled, getBehaviorValue } =
    ...
    if (isInfiniteScroll && filters.pagina && filters.pagina > 1) {
      setSales((prev) => {
        ...
        const newSales = salesData.data.filter(
          (newSale) =>
            !prev.some((existingSale) => existingSale.id === newSale.id)
        );
          ...
          <section className="flex items-center justify-between gap-2 md:gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isFeatureEnabled("refreshButton") && (
                ...
                >
                  <RefreshCcw
                    className={`size-4 ${isRefetchingSales || isFetching ? "animate-spin" : ""}`}
                  />
                </TooltipButton>
              )}
