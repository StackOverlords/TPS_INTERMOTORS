...
function ProductDetailTableInner(
  { isReadOnly = false, details }: ProductDetailTableProps,
  ref: React.Ref<ProductDetailTableRef>
) {
    ...
    () => [
      {
        ...
        minSize: 20,
        cell: ({ row }) => (
          <span className="text-center block">{row.index + 1}</span>
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => {
          ...
          return (
            <EditablePrice
              value={basePrice}
              onSubmit={(value) =>
                updateCustomPrice(product.id, value as number)
              }
              className="w-full"
              buttonClassName="w-full"
        ...
        cell: ({ row, getValue }) => {
          ...
          return (
            <Input
              ...
              onChange={(e) =>
                !isReadOnly &&
                updateCustomBrand(row.original.product.id, e.target.value)
              }
              autoSelectOnFocus={true}
              disabled={isReadOnly}
            />
