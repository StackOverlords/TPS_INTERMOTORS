...
import { useQuotationGetById } from "@/modules/quotations/hooks/useQuotationGetById";
import type { QuotationGetById } from "@/modules/quotations/types/quotationGet.types";

/**
 * Selector de cotizaciones, AGNÓSTICO AL CONTENEDOR.
 *
 ...
 */
export interface QuotationSelectorConfig {
  /** Desde dónde se abrió. Hoy informativo. */
  context: string;
}

export interface QuotationSelectorProps {
  config: QuotationSelectorConfig;
  /** Cotización elegida, ya cargada completa. El contenedor decide qué hacer. */
  onSelectQuotation: (quotation: QuotationGetById) => void;
  onClose: () => void;
}
  ...
  useEffect(() => {
    if (quotationFullData && selectedQuotationId) {
      const sendDataAndClose = async () => {
        ...
        onSelectQuotation(quotationFullData);
      };

      sendDataAndClose();
    }
  }, [quotationFullData, selectedQuotationId, onSelectQuotation]);
