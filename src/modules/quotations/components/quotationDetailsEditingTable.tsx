...
function QuotationDetailsEditingTableInner(
  ...
  ref: React.Ref<QuotationDetailsEditingTableRef>
) {
  ...
  const handleDeleteSuccess = (_data: unknown, detailId: number) => {
    const deletedItem = products.find(
      (p) => p.id_detalle_cotizacion === detailId
    );
  ...
  const { mutate: deleteQuotationDetail, isPending: isDeleting } =
          ...
          return (
            <div className="flex items-center">
              <Input
                ...
                value={description ?? ""}
                onChange={(e) =>
                  updateDescription(item.id_producto, e.target.value)
                }
                ref={refToAssign}
        ...
        cell: ({ getValue, row }) => {
          ...
          return (
            <EditablePrice
              value={basePrice}
              onSubmit={(value) =>
                updatePrice(item.id_producto, value as number)
              }
              className="w-full"
              buttonClassName="w-full"
        ...
        cell: ({ row }) => {
          ...
          return (
            <EditablePrice
              value={subtotal}
              onSubmit={(value) =>
                updateCustomSubtotal(item.id_producto, value as number)
              }
              className="w-full"
...
const QuotationDetailsEditingTable = forwardRef(
  QuotationDetailsEditingTableInner
) as React.ForwardRefExoticComponent<
  QuotationDetailsEditingTableProps &
    React.RefAttributes<QuotationDetailsEditingTableRef>
>;
