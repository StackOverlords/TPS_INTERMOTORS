...
const CreateTransfer = () => {
  ...
  const onSubmit = (data: TransferCreate) => {
    ...
    createTransfer(adjustedData, {
      ...
      onError: (error: unknown) => {
        handleError({
          error,
          customTitle: "No se pudo crear la transferencia",
        });
      },
    });
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      transferBranchesData.data.length > 0
    ) {
      const firstActiveBranch = transferBranchesData.data.find(
        (branch) => branch.activo === "SI"
      );
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Registra una nueva transferencia entre sucursales
                    </p>
                  </div>
                </div>
              </div>
              ...
              <CardContent className="flex-1 min-h-0 flex flex-col">
                {transferDetailsHook.details.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">
                      Haz clic en "Agregar" para añadir productos desde una
                      compra
                    </p>
                  </div>
                ) : (
