...
const EditTransfer = () => {
  ...
  useEffect(() => {
    if (
      ...
      setValue(
        "fecha",
        transferData.fecha?.split("T")[0] || format(new Date(), "yyyy-MM-dd")
      );
      ...
      setValue(
        "responsable",
        transferData.responsable?.id || Number(user?._id)
      );
      ...
      const transformedDetails = transferData.detalles
        ...
        .map((detalle) => ({
          ...
          precio_entrada_venta_alt: parseFloat(
            detalle.precio_entrada_venta_alt
          ),
  ...
  const validateBeforeSubmit = (): boolean => {
    ...
    if (transferDetailsHook.details.length === 0) {
      setError("detalles", {
        type: "manual",
        message:
          "Debes agregar al menos un producto para realizar una transferencia",
      });
      showErrorToast({
        title: "Sin productos",
        description:
  ...
  const onSubmit = async (data: TransferCreate) => {
    ...
    try {
      ...
      const uniqueDetails = transferDetailsHook.details.reduce(
        (acc: any[], detalle: any) => {
      ...
      const transformedDetalles = uniqueDetails.map((detalle: any) => {
        return {
          ...
          incremento_p_entrada_venta: Number(
            detalle.incremento_p_entrada_venta || 0
          ),
          incremento_p_entrada_venta_alt: Number(
            detalle.incremento_p_entrada_venta_alt || 0
          ),
        };
      });
      ...
      handleError({
        error,
        customTitle: "No se pudo actualizar la transferencia",
      });
    }
  };
          ...
          <Card className="shadow-none flex-shrink-0">
            <CardContent className="py-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                ...
                <div className="w-full">
                  <Label htmlFor="sucursal_origen" className="text-xs">
                    Sucursal Origen *
                  </Label>
                  <Input
                    id="sucursal_origen"
                ...
                <div className="w-full">
                  <Label htmlFor="sucursal_destino" className="text-xs">
                    Sucursal Destino *
                  </Label>
                  <Controller
                  ...
                  {errors.sucursal_destino && (
                    <p className="text-destructive text-xs mt-0.5">
                      {errors.sucursal_destino.message}
                    </p>
                  )}
                </div>

                <div className="w-full">
                  <Label htmlFor="nroComprobante" className="text-xs">
                    N° Comprobante
                  </Label>
                  <Input
                    id="nroComprobante"
                ...
                <div className="w-full">
                  <Label htmlFor="comentarios" className="text-xs">
                    Comentarios
                  </Label>
                  <Textarea
                    id="comentarios"
            ...
            <CardContent className="flex-1 min-h-0 flex flex-col">
              {transferDetailsHook.details.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Haz clic en "Agregar producto" para añadir productos
                  </p>
                </div>
              ) : (
                <>
                  <div className="flex-1 min-h-0 overflow-auto">
                    <TransferDetailTable
                      ...
                      onUpdateCostoEntrada={
                        transferDetailsHook.updateCostoEntrada
                      }
                      onUpdatePrecioSalida={
                        transferDetailsHook.updatePrecioSalida
                      }
                      onUpdatePrecioEntradaVenta={
                        transferDetailsHook.updatePrecioEntradaVenta
                      }
                      onUpdatePrecioEntradaVentaAlt={
                        transferDetailsHook.updatePrecioEntradaVentaAlt
                      }
                      onUpdateIncrementoPrecioEntradaVenta={
                        transferDetailsHook.updateIncrementoPrecioEntradaVenta
                      }
                  ...
                  <div className="flex justify-between items-center px-2 pt-2 flex-shrink-0">
                    ...
                    <span className="font-bold text-emerald-600">
                      {formatCurrency(transferDetailsHook.getTotal())}
                    </span>
                  </div>
                </>
              )}
