...
const TransferListScreen = () => {
    ...
    if (isInfiniteScroll && filters.pagina && filters.pagina > 1) {
      setTransfers((prev) => {
        ...
        const newTransfers = transfersData.data.filter(
          (newTransfer: TransferGetAll) =>
            !prev.some(
              (existingTransfer) => existingTransfer.id === newTransfer.id
            )
        );
  ...
  const { mutate: sendTransfer, isPending: isSending } = useSendTransfer();

  const { mutate: acceptTransfer, isPending: isAccepting } =
    useAcceptTransfer();

  const { mutate: refuseTransfer, isPending: isRefusing } = useRefuseTransfer();
  ...
  return (
    <main className="h-full p-2 gap-2 flex flex-col">
      ...
      >
        ...
        <div className="bg-background rounded-lg border border-border flex-1 overflow-auto">
          <TransferListTable
