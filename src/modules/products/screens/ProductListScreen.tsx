...
const ProductListScreen = () => {
  ...
  useEffect(() => {
    const defaultMode = getBehaviorValue<"realtime" | "manual">(
      "defaultSearchMode"
    );
    if (defaultMode) {
  ...
  const handleDeleteError = (error: unknown, productId: number) => {
    handleError({
      error,
      customTitle: `Error al eliminar el producto #${productId}`,
    });
  };
  ...
  const getStockColor = (stock: number, stock_min: number) => {
    ...
    const criticalThreshold =
      getBehaviorValue<number>("stockCriticalThreshold") ?? 5;
  ...
  const handleAddSelectedToCart = useCallback(() => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (selectedProducts.length > 0) {
          tableShoppingCartRef.current?.focusQuantityInputByProductId(
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-48">
                  {isFeatureEnabled("viewDetails") && (
                    <DropdownMenuItem
                      onClick={() => handleProductDetail(row.original)}
                    >
        ...
        cell: ({ getValue }) => {
          const stockResto =
            typeof getValue() === "string"
              ? parseFloat(getValue() as string)
              : getValue<number>();
          const stockRestoDisplay = isFinite(stockResto)
            ? stockResto.toFixed(0)
            : "0";
          return (
  ...
  const { table, resetAll } = useCustomTable({
    data: products,
    columns,
    ...
    enableColumnOrdering:
      getTableBehaviorValue("enableColumnReordering") ?? true,
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isFeatureEnabled("refreshButton") && (
              <TooltipButton
                ...
                tooltip={
                  config?.features?.refreshButton?.description ||
                  "Recargar productos"
                }
              >
                <RefreshCcw
                  className={`size-4 ${isRefetchingProducts || isFetching ? "animate-spin" : ""}`}
                />
              </TooltipButton>
            )}
            ...
            {isFeatureEnabled("resetTableButton") && (
              <TooltipButton
                ...
                tooltip={
                  config?.features?.resetTableButton?.description ||
                  "Resetear tabla"
                }
              >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="space-y-1 text-muted-foreground text-xs">
                        ...
                        <p>
                          ...
                          {COMMANDS["tableAndFilters.filter2"].description}:
                          Descripción
                        </p>
                        <p>
                          <ShortcutKey combo={filter3Keys} />
                          {COMMANDS["tableAndFilters.filter3"].description}:
                          Cod. OEM
                        </p>
                        <p>
                          <ShortcutKey combo={filter4Keys} />
                          {COMMANDS["tableAndFilters.filter4"].description}:
                          Cod. Upc
                        </p>
                        <p>
                          <ShortcutKey combo={nextFilterKeys} />
                          {COMMANDS["tableAndFilters.nextFilter"].description}
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex flex-col space-y-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="space-y-1 text-muted-foreground text-xs">
                            ...
                            <p>
                              <ShortcutKey combo={hotkeys.deactivate ?? ""} />{" "}
                              Salir de tabla{" "}
                            </p>
                            <p>
                              <ShortcutKey combo={hotkeys.moveUp ?? ""} /> /{" "}
                              <ShortcutKey combo={hotkeys.moveDown ?? ""} />{" "}
                              Navegar filas{" "}
                            </p>
                            <p>
                              <ShortcutKey combo={hotkeys.navigate ?? ""} />{" "}
                              Cambiar columna
                            </p>
                          </div>
                        </div>
                        <div className="space-y-1.5">
                          ...
                          <div className="space-y-1 text-muted-foreground text-xs">
                            ...
                            <p>
                              <ShortcutKey
                                combo={hotkeys.secondaryAction ?? ""}
                              />{" "}
                              Agregar al carrito{" "}
                            </p>
                            <p>
                              <ShortcutKey combo={"ctrl+d"} /> Abrir modal de
                              producto{" "}
                            </p>
                          </div>
                        </div>
            ...
            <div className="flex-1 min-h-0">
              {isInfiniteScroll ? (
                ...
                >
                  ...
                  >
                    <CustomizableTable
                      ...
                      tableRef={tableRef}
                      focused={isFocused}
                      keyboardNavigationEnabled={isFeatureEnabled(
                        "keyboardNavigation"
                      )}
                      enableColumnReordering={
                        getBehaviorValue<boolean>("enableColumnReordering") ??
                        true
                      }
                      enableSorting={false}
                ...
                <CustomizableTable
                  ...
                  enableColumnReordering={
                    getBehaviorValue<boolean>("enableColumnReordering") ?? true
                  }
                  enableSorting={false}
                  onDragEnd={handleDragEnd}
                  onDragStart={handleDragStart}
