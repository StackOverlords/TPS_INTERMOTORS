...
interface ImageViewerProps {
  ...
  isLoading?: boolean;
  allowEdit?: boolean; // prop para controlar si se muestra el botón de editar y se habilitan las funciones de edición
}
...
export function ImageViewer({
  ...
  // Ref para limpiar objectURLs creados por este componente
  ...
  // Sync de imagen — la conversión a blob: debe ser EAGERLY
  // para que el canvas no quede tainted
  useEffect(() => {
    ...
    if (currentImage?.startsWith("blob:")) {
      ...
      blobUrlsRef.current.delete(currentImage);
    }
    ...
    // URL HTTP → convertir a blob: URL para evitar canvas tainted
    if (imageSrc.startsWith("http://") || imageSrc.startsWith("https://")) {
      fetchImageAsBlobUrl(imageSrc)
        .then((blobUrl) => {
        ...
        .catch(() => {
          // Fallback: usar URL directa (canvas no funcionará pero al menos muestra la imagen)
          toast.error("No se pudo preparar la imagen para edición");
          setCurrentImage(imageSrc);
        });
    } else {
      // data: o blob: ya generado — asignar directo
      setCurrentImage(imageSrc);
      setHistory([]);
      setHistoryIndex(-1);
      setImageState(initialState);
  ...
  const loadImageSafely = useCallback(async (url: string): Promise<string> => {
    // data: y blob: → usar canvas directamente, nunca están tainted
    ...
    // URL http:// llegando aquí significa que syncImage falló — fallback via fetch
    return fetchImageAsBlobUrl(url).then((blobUrl) => {
      blobUrlsRef.current.add(blobUrl);
      return loadImageSafely(blobUrl); // recursión única: blob: → canvas
    });
  }, []); // blobUrlsRef es ref estable
  ...
  const handleRotateCcw = () =>
    updateState({ rotation: (imageState.rotation - 90 + 360) % 360 });
  ...
  const applyTransformations = useCallback(async (): Promise<string> => {
    ...
    try {
      ...
      throw new Error(
        `Transform error: ${error instanceof Error ? error.message : "Error desconocido"}`
      );
    }
  ...
  const applyCrop = useCallback(async () => {
    ...
    try {
      ...
      const cropX =
        Math.min(cropArea.startX, cropArea.endX) -
        (imgRect.left - containerRect.left);
      const cropY =
        Math.min(cropArea.startY, cropArea.endY) -
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Mostrar info
      if (result.skipped) {
        toast.success(result.reason || "Imagen lista", {
          id: downloadToast,
          duration: 4000,
        });
      } else {
      ...
      // Nombre con calidad
      const baseName =
        imageMetadata?.fileName?.replace(/\.[^/.]+$/, "") || "imagen";
  ...
  const getTransformStyle = (): React.CSSProperties => ({
    ...
    cursor:
      needsDrag && !isCropping && !magnifierActive
        ? isDraggingImage
          ? "grabbing"
          : "grab"
        : undefined,
  ...
  const ToolButton = ({
    ...
    <Tooltip>
      ...
      <TooltipContent
        side={side}
        className="bg-zinc-800 text-white border-zinc-700 text-xs z-50"
      >
        {label}
      </TooltipContent>
    </Tooltip>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="max-w-[95vw] w-full h-[90vh] p-0 bg-zinc-900 border-zinc-800 overflow-hidden z-[9999]"
      >
        ...
        >
          ...
          <div
            className={cn(
              ...
              isFullscreen &&
                showFullscreenControls &&
                !isEditMode &&
                "opacity-90 translate-y-0 absolute top-0 left-0 right-0 bg-zinc-900/95 backdrop-blur-sm z-10"
            )}
          >
            <div className="flex items-center gap-4">
              ...
              {imageMetadata?.fileName && (
                <div className="flex items-center gap-1 px-2 py-1 rounded bg-zinc-800 text-zinc-400 text-xs">
                  <span>
                    {imageMetadata.fileName} - {imageMetadata.fileExtension}
                  </span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              {!isEditMode && currentImage && allowEdit !== false && (
                <>
                  <ToolButton
                    icon={Edit}
                    label="Editar imagen"
          ...
          <div className="flex flex-1 overflow-hidden min-h-0">
            ...
            <div
              ...
              onMouseDown={
                isCropping ? handleCropMouseDown : handleImageMouseDown
              }
              onMouseMove={(e) => {
                  ...
                  >
                    ...
                    {getCropOverlayStyle() && (
                      <div className="absolute -top-6 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-zinc-800 text-white text-xs rounded">
                        {Math.round(getCropOverlayStyle()!.width as number)} x{" "}
                        {Math.round(getCropOverlayStyle()!.height as number)}
                      </div>
                    )}
                  </div>
                </>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onClick={() =>
                              updateState({
                                zoom: Math.max(0.5, imageState.zoom - 0.25),
                              })
                            }
                            className="size-8 text-zinc-400 hover:text-white hover:bg-zinc-700/50"
                          >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              updateState({
                                zoom: Math.min(4, imageState.zoom + 0.25),
                              })
                            }
                            className="size-8 text-zinc-400 hover:text-white hover:bg-zinc-700/50"
                          >
                        ...
                        {isCropping && (
                          <div className="space-y-2">
                            <p className="text-xs text-zinc-400 text-center">
                              Arrastra sobre la imagen para seleccionar el área
                              a recortar
                            </p>
                            <div className="flex gap-2">
                              <Button
          ...
          <div
            className={cn(
              ...
              isFullscreen &&
                showFullscreenControls &&
                !isEditMode &&
                "opacity-90 translate-y-0 absolute bottom-0 left-0 right-0 bg-zinc-900/95 backdrop-blur-sm z-10"
            )}
          >
              ...
              {!isEditMode && (
                <>
                  ...
                  <ToolButton
                    ...
                    label="Acercar"
                    onClick={() =>
                      updateState({ zoom: Math.min(4, imageState.zoom + 0.25) })
                    }
                  />
                  <div className="w-px h-6 bg-zinc-700 mx-2" />
                  <ToolButton
                  ...
                  <ToolButton
                    ...
                    onClick={handleFlipX}
                    active={imageState.flipX}
                  />
                  <ToolButton
                    icon={FlipVertical}
                    label="Voltear vertical"
                    onClick={handleFlipY}
                    active={imageState.flipY}
                  />
                  <div className="w-px h-6 bg-zinc-700 mx-2" />
                </>
              )}
