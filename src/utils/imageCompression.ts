...
export interface CompressionOptions {
  ...
  export async function getImageInfo(base64Data: string): Promise<ImageInfo> {
    try {
      return await invoke<ImageInfo>('get_image_info', { base64Data });
    } catch (error) {
  ...
  export async function smartCompressToWebP(
    ...
    try {
      ...
      const originalSize = (base64Data.split(',')[1]?.length || base64Data.length) * 0.75;
      ...
      const compressed = await invoke<string>('compress_image_to_webp', {
        base64Data,
        quality: quality / 100,
        effort,
      });
      
      const compressedSize = (compressed.split(',')[1]?.length || compressed.length) * 0.75;
  ...
  export async function forceCompressToWebP(
    ...
    try {
      return await invoke<string>('compress_image_to_webp', {
        base64Data,
        quality: quality / 100,
        effort,
      });
